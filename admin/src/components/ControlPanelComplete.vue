...
async function exportDocx() {
    ...
    for (let j = 0; j < questions.length; j++) {
        ...
        children.push(new Paragraph({
            "children": [new TextRun((j + 1) + ") " + question.querySelectorAll(".questionTitle")[0].textContent + " ....... " + question.querySelectorAll(".questionTitle")[1].textContent)]
        }));
    }
...
<template>
    <div v-for="question in questions" :key="question['id']" @click="$event => {if (!$event.target.closest('button') && member == 'admin' && !creatingTest) $event.currentTarget.querySelector('.uploader-name').classList.toggle('d-none');if (creatingTest) $emit('changeChosenQuestions', question['id'])}" class="question border border-2 d-flex flex-column p-2 bg-body-tertiary" :class="{'border-dark': creatingTest, 'chosen': chosenQuestions.includes(question['id'])}">
        ...
        <div v-if="((member == routeParams.game || !routeParams.grade.includes('secondary')) && question['uploader'].toLowerCase() == username.toLowerCase()) || member == 'admin'" class="d-flex flex-row justify-content-center pt-2 px-1 gap-1" :class="{'bg-success-subtle': question['level'] == 'easy', 'bg-warning-subtle': question['level'] == 'medium', 'bg-danger-subtle': question['level'] == 'hard'}">
        ...
        <div class="modal-dialog modal-dialog-scrollable modal-xl">
            <div class="modal-content bg-light">
                <div class="modal-body">
                    ...
                    <button class="btn btn-danger btn-close float-end" data-bs-dismiss="modal" aria-label="close" data-cy="closeComplete"></button>
