describe("print info", () => {
    context("prints lessons correctly", () => {
        it("returns the lesson's questions", function () {
            ...
            cy.request("http://localhost/info/functions/printInfo.php?grade=grade-4-first-term&game=complete&unit=1.1-Adaptation-and-Survival").then(resp => {
                resp = JSON.parse(resp.body);
                expect(resp).to.be.instanceOf(Object);
                expect(resp[0]["wrongAnswer"]).to.be.a("string");
                expect(resp[0]["rightAnswer"]).to.be.a("string");
            });
            cy.request("http://localhost/info/functions/printInfo.php?grade=grade-4-first-term&game=match&unit=1.1-Adaptation-and-Survival").then(resp => {
                resp = JSON.parse(resp.body);
                expect(resp).to.be.instanceOf(Object);
            ...
            cy.request("http://localhost/info/functions/printInfo.php?grade=grade-4-first-term&game=right-or-wrong&unit=1.1-Adaptation-and-Survival").then(resp => {
                resp = JSON.parse(resp.body);
                expect(resp).to.be.instanceOf(Object);
                expect(resp[0]["question"]).to.be.a("string");
