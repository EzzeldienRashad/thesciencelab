...
describe("<ChooseGame />", () => {
    ...
    it("renders questions and answers correctly", function () {
        cy.get("[data-cy=question]").should("have.text", "What is a?");
        cy.get("[data-cy=choices] button").should("have.length", 4);
        cy.get("[data-cy=choices] button").eq(1).should("have.text", "b");
        cy.then(() => this.props.answeredQuestions.value++);
        cy.get("[data-cy=question]").should("have.text", "What is b?");
    });
    it("calls functions and performs actions correctly", function () {        
        cy.get("[data-cy=choices] button").eq(0).click().then(() => {
            expect(this.props.changeAnswered).to.have.been.called;
            expect(this.props.addRightAnswer).to.have.been.called
        });
        cy.get("[data-cy=choices] button").eq(1).should("have.css", "pointer-events", "none");
        cy.then(() => {
            this.props.answeredQuestions.value++;
            this.props.answered.value = false;
        });
        cy.get("[data-cy=choices] button").eq(2).click().then(() => {
