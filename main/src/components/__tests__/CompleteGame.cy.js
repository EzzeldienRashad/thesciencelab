...
describe("<CompleteGame />", () => {
    ...
    it("renders questions and answers correctly", function () {
        ...
        cy.get("[data-cy=questions] input").should("have.length", 2);
    ...
    it("calls functions and performs actions correctly", function () {     
        cy.get("[data-cy=questions] input").eq(0).type("love");   
        cy.get("[data-cy=questions] input").eq(1).type("apple");
        ...
        cy.get("[data-cy=questions] input").eq(1).clear().type("BaNanA");
        cy.contains("check").click().then(() => {
