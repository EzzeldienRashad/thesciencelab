...
describe("<RightOrWrongGame />", () => {
    ...
    it("renders questions and answers correctly", function () {
        cy.get("[data-cy=question]").should("have.text", "Is it right?");
        cy.get("[data-cy=choices] button img").should("have.length", 2);
        cy.then(() => this.props.answeredQuestions.value++);
        cy.get("[data-cy=question]").should("have.text", "Is it wrong?");
    });
    it("calls functions and performs actions correctly", function () {        
        cy.get("[data-cy=choices] button").eq(0).click().then((button) => {
        ...
        cy.get("[data-cy=question]").should("have.class", "text-success");
        cy.then(() => {
